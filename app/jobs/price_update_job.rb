...
class PriceUpdateJob < ApplicationJob
  ...
  def perform()
  	puts "Opening quote2.py..."
    _output = Open3.capture2('python3.6 quote2.py')

    puts "Opened quote2.py. Getting main part..."
    _output = _output[0]
    ...
    if (_output.include?("ASX") || !(_output.include?("exception")) )
	    ...
	    _output.each_with_index {|_entry,_index|
	    	if (_entry.include?("ASX") && (_output.size / 2 == Security.count))
